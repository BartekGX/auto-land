				...
				<Reorder.Group
					...
					{data.map((item, index) => (
						<Reorder.Item
							...
							<div>
								<div className=" flex gap-2">
									<div className="flex flex-wrap flex-col border border-neutral-600 rounded-lg p-1">
										<span className="text-gray-400">rok produkcji</span>
										<p className="font-medium text-gray-300">{item.info.year}</p>
									</div>
									<div className="flex flex-wrap flex-col border border-neutral-600 rounded-lg p-1">
										<span className="text-gray-400">przebieg</span>
										<p className="font-medium text-gray-300">{item.info.process} km</p>
									</div>
									<div className="flex flex-wrap flex-col border border-neutral-600 rounded-lg p-1">
										<span className="text-gray-400">moc</span>
										<p className="font-medium text-gray-300">{item.info.power} KM</p>
									</div>
									<div className="flex flex-wrap flex-col border border-neutral-600 rounded-lg p-1">
										<span className="text-gray-400">pojemność silnika</span>
										<p className="font-medium text-gray-300">{item.info.capacity} cm3</p>
									</div>
									<div className="flex flex-wrap flex-col border border-neutral-600 rounded-lg p-1">
										<span className="text-gray-400">rodzaj paliwa</span>
										<p className="font-medium text-gray-300">{item.info.fuel}</p>
									</div>
								</div>
							</div>
						</Reorder.Item>
